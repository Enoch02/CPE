...
fun ScheduleScreen(
    ...
    LaunchedEffect(key1 = viewModel.today) {
        pagerState.animateScrollToPage(viewModel.today)
    }
    Column(modifier = modifier) {
        TabRow(selectedTabIndex = pagerState.currentPage) {
            viewModel.days.forEachIndexed { index, day ->
                Tab(
        ...
        HorizontalPager(
            count = viewModel.days.size,
            state = pagerState,
            content = { dayInt ->
                val classesToday by remember { mutableStateOf(viewModel.filterClasses(dayInt)) }
